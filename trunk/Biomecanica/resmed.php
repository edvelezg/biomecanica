...
<html>
	...
	<body>
		<h2> Resultados de Busqueda de Medicos</h2>
		<hr>
...
<?php
	...
	if (!isset($_GET['page']))
	{
		if (!$_POST['searchtype'] || !$_POST['searchterm'])
		{
			...
			echo '<br><br>';
			printf("<a class='button' href=\"busmed.php\">Volver</a>");
			echo '<br><br></br>';
			require("footer.php");
			exit;
		}
	...
	if ($searchtype == "id_med") {
		$query = "select * from medicos where ".$searchtype." = '".$searchterm."' ";
		$pagingQuery = "LIMIT $offset, $rowsPerPage";
		$result = mysql_query($query . $pagingQuery) or die('Ocurrio un error en la busqueda de medicos');
		$numrows = mysql_num_rows($result);

		$numquery = "select * from medicos where ".$searchtype." = '".$searchterm."' ";
		$all = mysql_query($numquery) or die('Ocurrio un error al contar los resultados');
		$allrows = mysql_num_rows($all); 	
	} 
	else {
		$query = "select * from medicos where ".$searchtype." like '".$searchterm."%' ";
		$pagingQuery = "LIMIT $offset, $rowsPerPage";
		$result = mysql_query($query . $pagingQuery) or die('Ocurrio un error en la busqueda de medicos');
		$numrows = mysql_num_rows($result);

		$numquery = "select * from medicos where ".$searchtype." like '".$searchterm."%' ";
		$all = mysql_query($numquery) or die('Ocurrio un error al contar los resultados');
		$allrows = mysql_num_rows($all); 	
	}
	...
	echo "<p>Numero de Medicos encontrados: ".$allrows."</p>";
	
		echo "<table border=1 cellpadding='10' width='600'>\n";
		#Vista Gral
		echo "<tr><th width=220><b>Nombre</b></th><th><b>Institucion</b></th></tr>\n";
		while ( $row = mysql_fetch_array($result) ) {		
			printf("<tr><td><a href=\"Medicos.php?nom=%s\">%s</a></td><td>%s</td></tr>\n",
			$row["nombre"], ucwords($row["nombre"]), $row["institucion"]);
		}
